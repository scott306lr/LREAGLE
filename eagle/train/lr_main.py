...
# from ..model.llama.lrdraft_ttt import LRDraft
# from ..model.llama.configuration_llama import LlamaConfig
...
baseconfig = AutoConfig.from_pretrained(config.base_model_name_or_path)
# baseconfig = AutoConfig.from_pretrained("meta-llama/Llama-2-7b-chat-hf")

head = torch.nn.Linear(baseconfig.hidden_size, baseconfig.vocab_size, bias=False)
embed_tokens = torch.nn.Embedding(baseconfig.vocab_size, baseconfig.hidden_size, baseconfig.pad_token_id)
...
try:
    ...
    with safe_open(os.path.join(args.basepath, head_path),
        ...
        head_tensor = tensor_slice[:, :hidden_dim].float()
    with safe_open(os.path.join(args.basepath, embed_path),
                    framework="pt",
                    device="cpu") as f:
        tensor_slice = f.get_slice("model.embed_tokens.weight")
        vocab_size, hidden_dim = tensor_slice.get_shape()
        embed_tensor = tensor_slice[:, :hidden_dim].float()
except:
    ...
    head_tensor = weights["lm_head.weight"].float()
    # embed
    weights = torch.load(os.path.join(args.basepath, embed_path))
    embed_tensor = weights["model.embed_tokens.weight"].float()

head.weight.data = head_tensor
head.eval()

embed_tokens.weight.data = embed_tensor
embed_tokens.eval()
...
# config = LRDraft.from_pretrained(train_config["config_path"])

# if args.lrmod:
#     model = LRModel(config, load_emb=True, path=args.basepath)
# else:
#     model = Model(config, load_emb=True, path=args.basepath)

# model = LRDraft(config)
# model.set_embedding_and_head(embed_tokens, head)
print("Loading the whole model...")
print("Disabling use_cache...")
config.use_cache=False
model = EagleModelLlama(config)
draft_model = model.draft_model
draft_model.set_head_and_embed(head, embed_tokens)
draft_model = deepcopy(model.draft_model)

print("Delete base model, keeping only draft...")
del model
model = draft_model

criterion = nn.SmoothL1Loss(reduction="none")
# optimizer = optim.AdamW(model.parameters(), lr=train_config["lr"], betas=(train_config["b1"], train_config["b2"]))
optimizer = apex.optimizers.FusedAdam(model.parameters(), lr=train_config["lr"], betas=(train_config["b1"], train_config["b2"]))
...
for epoch in range(num_epochs + 1):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        with accelerator.accumulate(model):
            ...
            with torch.no_grad():
                # back to fp32
                target_head = model.module.lm_head(data["target"].to(model.module.lm_head.weight.dtype))
            ...
            # grokfast
            # grads = gradfilter_ema(model.module, grads=grads, alpha=alpha, lamb=lamb)
    ...
    if (epoch + 1) % train_config["save_freq"]:
        ...
        if accelerator.is_local_main_process:
            ...
            accelerator.save_model(model, f"{args.cpdir}/model_{epoch}")
            draft_model.set_head_and_embed(lm_head, embed_tokens)
