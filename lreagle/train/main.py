...
from ..model.llama.lrdraft import LRDraft
...
for epoch in range(num_epochs + 1):
    ...
    for batch_idx, data in enumerate(tqdm(train_loader)):
        ...
        with accelerator.accumulate(model):
            ...
            with torch.no_grad():
                # back to fp32
                target_head = model.module.head(data["target"].to(model.module.head.weight.dtype))
                target_p = nn.Softmax(dim=2)(target_head)
                target_p = target_p.detach()
            out_head = model.module.head(predict)
    ...
    if (epoch + 1) % train_config["save_freq"]:
        ...
        for batch_idx, data in enumerate(tqdm(test_loader)):
            with torch.no_grad():
                if batch_idx < 10:
                    acces = getkacc(model, data, model.module.head, max_length=5, dtype=model.module.head.weight.dtype)
        ...
        if accelerator.is_local_main_process:
            ...
            model.module.discard_embedding_and_head()
            accelerator.save_model(model, f"{args.cpdir}/model_{epoch}")
            model.module.set_embedding_and_head(embed_tokens, head)
